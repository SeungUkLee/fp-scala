...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //((left union right) union that) incl elem -> time out
  def union(that: TweetSet): TweetSet = (left union (right union that)).incl(elem)

  def mostRetweeted: Tweet = {
    def maxRetweet(a: Tweet, b: Tweet): Tweet = {
    ...
    else if (left.isEmpty) maxRetweet(rightMost, elem)
    else if (right.isEmpty) maxRetweet(leftMost, elem)
    else maxRetweet(leftMost, maxRetweet(leftMost, elem))
  }
